...
@tracer.start_as_current_span("_NotifierUserStream")
class _NotifierUserStream:
    ...
    @tracer.start_as_current_span("_NotifierUserStream - __init__")
    def __init__(
        self,
    ...
    @tracer.start_as_current_span("_NotifierUserStream - update_and_fetch_deferreds")
    ...
    @tracer.start_as_current_span("_NotifierUserStream - remove")
    ...
    @tracer.start_as_current_span("_NotifierUserStream - count_listeners")
    def count_listeners(self) -> int:
        return len(self.listeners)

    @tracer.start_as_current_span("_NotifierUserStream - new_listener")
    def new_listener(self, token: StreamToken) -> "Deferred[StreamToken]":
        ...
        print("AAAAAAAAAAAAAAAAAAAAAA Número de listeners ativos: %d", len(self.listeners))
...
@tracer.start_as_current_span("Notifier")
class Notifier:
    ...
    @tracer.start_as_current_span("Notifier - __init__")
    ...
    @tracer.start_as_current_span("Notifier - add_replication_callback")
    ...
    @tracer.start_as_current_span("Notifier - add_new_join_in_room_callback")
    ...
    @tracer.start_as_current_span("Notifier - on_new_room_events")
    ...
    @tracer.start_as_current_span("Notifier - on_un_partial_stated_room")
    async def on_un_partial_stated_room(
        ...
        with PreserveLoggingContext():
            for listener in listeners:
                print("BBBBBBBBBBBBBBBBBBB Disparando callback do listener com token %s", current_token)
    ...
    @tracer.start_as_current_span("Notifier - notify_new_room_events")
    async def notify_new_room_events(
        ...
        the client streams.
        """
        with tracer.start_as_current_span("Notifier - notify_new_room_events2") as child: 
    ...
    @tracer.start_as_current_span("Notifier - create_pending_room_event_entry")
    ...
    @tracer.start_as_current_span("Notifier - _notify_pending_new_room_events")
    ...
    @tracer.start_as_current_span("Notifier - _on_updated_room_token")
    ...
    @tracer.start_as_current_span("Notifier - _notify_app_services")
    ...
    @tracer.start_as_current_span("Notifier - _notify_pusher_pool")
    ...
    @overload
    @tracer.start_as_current_span("Notifier - on_new_event")
    def on_new_event(
        self,
    ...
    def on_new_event(
        ...
        with tracer.start_as_current_span("Notifier - on_new_event2") as child: 
            if self.user_to_user_stream or self.room_to_user_streams:
                ...
                with tracer.start_as_current_span("Notifier - on_new_event - listeners") as child: 
                    if listeners:
                        with PreserveLoggingContext():
                            for listener in listeners:
                                listener.callback(current_token)
                                print("AAAAAAAAAAAAAAAAAAAAa Disparando callback do listener com token %s", current_token)
    ...
    @tracer.start_as_current_span("Notifier - on_new_replication_data")
    def on_new_replication_data(self) -> None:
        """Used to inform replication listeners that something has happened
        without waking up any of the normal user event streams"""
        self.notify_replication()

    @tracer.start_as_current_span("wait_for_events")
    async def wait_for_events(
        ...
        timeout fires.
        """
        with tracer.start_as_current_span("sync-notifier") as child:                
            ...
            with tracer.start_as_current_span("sync-timeout") as child:                
                if timeout:
                    end_time = self.clock.time_msec() + timeout

                    while not result:
                        with tracer.start_as_current_span("sync-notifier-not-result") as child:                
                            try:
                                ...
                                # is a new token.             
                                print("AAAAAAAAAAAAAAAAA Registrando listener no token %s", prev_token)               
                                ...
                                with tracer.start_as_current_span("sync-notifier-wait_for_events-woken") as child: 
                                    start = self.clock.time_msec()
                                    with PreserveLoggingContext():
                                        await listener
                                    duration = self.clock.time_msec() - start
                                    print("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAaa " + str(duration))
    
                                    current_token = user_stream.current_token

                                with tracer.start_as_current_span("sync-notifier-wait_for_events-result") as child: 
            ...
            with tracer.start_as_current_span("sync-notifier-result-none") as child: 
                if result is None:
    ...
    @tracer.start_as_current_span("get_events_for")
    ...
    @tracer.start_as_current_span("wait_for_stream_token")
    ...
    @tracer.start_as_current_span("_get_room_ids")
    ...
    @tracer.start_as_current_span("_is_world_readable")
    ...
    @tracer.start_as_current_span("remove_expired_streams")
    ...
    @tracer.start_as_current_span("_register_with_keys")
    ...
    @tracer.start_as_current_span("_user_joined_room")
    ...
    @tracer.start_as_current_span("add_lock_released_callback")
    ...
    @tracer.start_as_current_span("notify_lock_released")
