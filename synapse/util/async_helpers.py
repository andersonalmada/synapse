...
@tracer.start_as_current_span("timeout_deferred")
def timeout_deferred(
    ...
    @tracer.start_as_current_span("time_it_out")
    ...
    @tracer.start_as_current_span("convert_cancelled")
    ...
    @tracer.start_as_current_span("cancel_timeout")
    ...
    @tracer.start_as_current_span("success_cb")
    def success_cb(val: _T) -> None:
        if not new_d.called:
            new_d.callback(val)

    @tracer.start_as_current_span("failure_cb")
...
class AwakenableSleeper:
    ...
    @tracer.start_as_current_span("wake")
...
class DeferredEvent:
    ...
    @tracer.start_as_current_span("wait")
